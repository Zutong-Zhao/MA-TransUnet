...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if x != self.output_size[0] or y != self.output_size[1]:
            image = zoom(image, (self.output_size[0] / x, self.output_size[1] / y), order=3)#//放缩图像，order为插值方法  # why not 3?
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            if x != self.img_size or y != self.img_size:
                image = zoom(image, (self.img_size / x, self.img_size / y), order=3)#//放缩图像，order为插值方法   # why not 3?
