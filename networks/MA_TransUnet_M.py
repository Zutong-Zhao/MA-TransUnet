...
from networks.MISS_Former_Block import TransformerBlock
...
class Encoder(nn.Module):
    def __init__(self, image_size, in_dim, key_dim, value_dim, layers, head_count=1, token_mlp="mix_skip"):
        ...
        self.block1 = nn.ModuleList(
            [TransformerBlock(in_dim[0], heads[0], reduction_ratios[0], token_mlp) for _ in range(layers[0])]
        )
        self.norm1 = nn.LayerNorm(in_dim[0])

        self.block2 = nn.ModuleList(
            [TransformerBlock(in_dim[1], heads[1], reduction_ratios[1], token_mlp) for _ in range(layers[1])]
        )
        self.norm2 = nn.LayerNorm(in_dim[1])

        self.block3 = nn.ModuleList(
            [TransformerBlock(in_dim[2], heads[2], reduction_ratios[2], token_mlp) for _ in range(layers[2])]
        )
...
class MyDecoderLayer(nn.Module):
    def __init__(
        self, input_size, in_out_chan, head_count,heads, reduction_ratios, token_mlp_mode, n_class=9, norm_layer=nn.LayerNorm, is_last=False
    ):
        super().__init__()
        dims = in_out_chan[0]
