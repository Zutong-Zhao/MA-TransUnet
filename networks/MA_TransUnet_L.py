...
from networks.EF_ATT import FrequencyTransformerBlock
...
class Encoder(nn.Module):
    def __init__(self, image_size, in_dim, key_dim, value_dim, layers, head_count=1, token_mlp="mix_skip"):
        ...
        self.block1 = nn.ModuleList(
            [FrequencyTransformerBlock(in_dim[0], key_dim[0], value_dim[0], pyramid_levels=3,token_mlp='mix_skip') for _ in range(layers[0])]
        )
        self.norm1 = nn.LayerNorm(in_dim[0])

        self.block2 = nn.ModuleList(
            [FrequencyTransformerBlock(in_dim[1], key_dim[1], value_dim[1], pyramid_levels=3, token_mlp='mix_skip') for _ in range(layers[1])]
        )
        self.norm2 = nn.LayerNorm(in_dim[1])

        self.block3 = nn.ModuleList(
            [FrequencyTransformerBlock(in_dim[2], key_dim[2], value_dim[2], pyramid_levels=3, token_mlp='mix_skip') for _ in range(layers[2])]
        )
...
class MyDecoderLayer(nn.Module):
    def __init__(
        ...
        self.layer_former_1 = FrequencyTransformerBlock(out_dim, key_dim, value_dim, pyramid_levels=3, token_mlp='mix_skip')
        self.layer_former_2 = FrequencyTransformerBlock(out_dim, key_dim, value_dim, pyramid_levels=3, token_mlp='mix_skip')
