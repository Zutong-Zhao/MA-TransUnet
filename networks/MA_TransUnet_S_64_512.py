...
from networks.swin_block_64_512 import SwinTransformer
...
class Encoder(nn.Module):
    def __init__(self, image_size, in_dim, key_dim, value_dim, layers, head_count=1, token_mlp="mix_skip"):
        ...
        self.p1_ch = nn.Conv2d(256, 64, kernel_size=1)
        self.p2 = self.resnet_layers[5]
        self.p2_ch = nn.Conv2d(512, 128, kernel_size=1)
        self.p3 = self.resnet_layers[6]
        self.p3_ch = nn.Conv2d(1024, 256, kernel_size=1)

        patch_sizes = [7, 3, 3, 3]
        strides = [4, 2, 2, 2]
        padding_sizes = [3, 1, 1, 1]

        self.swin_transformer = SwinTransformer(image_size=image_size, in_chans=3)
        # patch_embed
...
class MyDecoderLayer(nn.Module):
    def __init__(
        ...
        self.layer = layer
        self.swin_transformer = SwinTransformer(image_size=512, in_chans=3)
        if not is_last:
...
class myFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        ...
        # Decoder
        d_base_feat_size = 16  # 16 for 512 input size, and 7 for 224
        in_out_chan = [
            [32, 64, 64, 64, 64],
            [128, 128, 128, 128, 128],
            [256, 256, 256, 256, 256],
